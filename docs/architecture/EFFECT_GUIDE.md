...
Adopted for HA Workspace on September 8, 2026. The connected implementation uses Effect v4 RC, one authenticated Confect/Atom browser runtime, local file capabilities, and durable revision jobs. Doppler injection and the configured Bun production build pass. The latest source suite passes 121 TypeScript tests and skips one optional local-runtime test. That optional test passed in the earlier Doppler-configured verification, alongside 21 Python document-helper tests; `SETUP.md` separates runtime/browser evidence from source checks. The linked API probes remain historical examples, not substitutes for application tests.
...
## Current composition and deliberate limits

The client-only workspace route mounts the session registry and one Confect runtime. `/api/session` returns an in-memory owner token and the public local backend URL; the server also sets an HttpOnly same-origin cookie for preview and download requests. Authentication confirmation gates subscriptions. Typed result streams preserve the previous successful view after a recoverable failure. The local issuer is a loopback-only prototype; remote authentication is not solved by the presence of a JWT.

The sidebar open preference uses `Atom.kvs` with a Boolean schema. Review and revision drafts use explicit IndexedDB Effects keyed by owner, file, and source version. Draft memory, saving acknowledgement, and write failure are distinct states. Autosave commands survive panel changes inside the session; credentials never enter that store. Canonical filter keys let equivalent catalog reads share their atom.
...
Use Effect Config at composition roots and injected services for configuration-dependent behavior. Doppler `ha-workspace` / `dev_personal` supplies all application environment values through `doppler run --no-fallback`; do not read app `.env` files or embed fallback secrets. Decode required values once, keep secrets redacted, and export only allowlisted public configuration to the browser. Doppler CLI authentication and runtime injection have been verified. Configured builds, import, backend deployment, the web server, and the worker use this path. Name use-case spans and record job IDs, file-version IDs, attempts, stage durations and outcomes. Read random values and time through the appropriate runtime services; inside Convex, use its deterministic environment and Confect services.

## The local Drive is an Effect adapter

The current `DriveStore` capability provides the working-copy boundary; the broader catalog/application design keeps provider operations behind focused Effect ports. Its operations return typed Effect values for listing metadata, reading versioned content, staging bytes, and exclusively publishing a new filename. The first live Layer reads `/home/akh/Projects/ha-workspace-data/fake-drive` and records immutable managed revisions. Read the provider root from Doppler configuration; application use cases accept IDs and validated references, never arbitrary absolute paths from the browser.
...
`atom-react-sample.tsx` and `atom-auth.mts` also passed typechecking with Confect next.21, React 19.2.4, and scheduler 0.27.0. These are isolated API probes. The React fixture uses an illustrative local URL, simplified public refs, and temporary draft state; production uses the confirmed endpoint, authenticated generated refs, revision checks, and durable draft storage. Those historical probes did not test live Convex, a browser mount, an authentication handshake, or Bun integration. Current application evidence is recorded separately in `SETUP.md`. `ATOM-VERIFICATION.md` records the exact checks and sources.
