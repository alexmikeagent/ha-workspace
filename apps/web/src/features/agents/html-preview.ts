...
function previewDocument(body: string) {
  ...
  return `<!doctype html><html><head><meta http-equiv="Content-Security-Policy" content="${PREVIEW_POLICY}"><meta charset="utf-8"><meta name="referrer" content="no-referrer"></head><body>${body}</body></html>`
}
...
export function sanitizeAgentHtml(source: string): string {
  ...
  return previewDocument(output.innerHTML)
}
